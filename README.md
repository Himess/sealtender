...
- **v7** (May 11, 2026, **shipped + deployed on Sepolia**) — **cUSDC-native bid escrow.** Full refactor of `BidEscrow` to accept ERC-7984 confidential USDC. `deposit(tenderId, externalEuint64, proof)` pulls cUSDC via `confidentialTransferFrom`; refund/slash use `confidentialTransfer`. Bond amounts never leak via on-chain events. New deployments: BidEscrow `0x94c51890…371C`, TenderFactory `0x18b75f82…7A14`, ConfidentialUSDC `0xcD124Ce2…A399`, MockUSDC `0xA5e15640…8B58`. Test suite restructured with `test/helpers/escrowSetup.ts` (mock cUSDC wrap + setOperator + deposit fixture). 329/329 tests pass. Frontend adds `/cusdc` onboarding route (mint → approve → wrap → setOperator); bid page now does encrypted-cUSDC deposit + submitBid as two sequential MetaMask popups. Aligns with the Zama Developer Program Builder Track requirement: "Integrate OpenZeppelin's Confidential Contracts."
